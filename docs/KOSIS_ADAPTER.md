...
## 2026-09-17 — Phase B 배치 적재 기록

| 표 | 항목 | 기간 | 행 수 | 결측 | snapshot ID 앞 8자리 | 비고 |
|---|---|---|---|---|---|---|
| `101/DT_1YL21281` T10 공원 | 인구 천명당 면적, 천㎡ | 2025 | 17 | 0 | `b0f7f9c9` | `1224/1236` 대응표 |
| `101/DT_1C96` T1 GRDP | 1인당 지역내총생산, 천원 | 1985~2024 | 633 | 0 | `b2e3a59f` | 구체계 exact, 지역별 시작연도 상이(정상) |
| `101/DT_1PE105` T00 사교육비 | 월평균, 만원 | 2009~2025 | 284 | 0 | `74a0f5c0` | 구체계 exact |
| `116/DT_MLTM_5498` T4 자동차 | 등록대수 총계, 대 | 2018-01~2026-07(월별) | 1,749 | 0 | `bd2d9630` | 3중 분류(objL1~3), 월별→연평균 집계, 자동차 전용 대응표 17건 |
| `101/DT_1B81A19` T1 출생성비 | 총출생성비 | 1990~2025 | 612 | 29 | `26dc6a64` | 울산 1990~1996·세종 1990~2011 승격 전 결측(`-` 보존) |
| `101/DT_1K52F01` T1 사업체수 | 전산업, 개 | 2020~2024 | 85 | 0 | `443a65f9` | 3중 분류(objL1~3) |

제외: `106/DT_106N_03_02000xx` PM10/PM2.5 시도별 — 파라미터 조합을 바꿔도 API가 빈 응답(`30`)만 반환해 적재 불가. 대안은 에어코리아 직접 수집(Phase D).

교훈: (1) 분류가 2~3중인 표는 `OBJ_ID_SN` 순서대로 objL1~3을 모두 채워야 한다(누락 시 `20` 에러). (2) 응답 `PRD_SE`는 요청 코드와 다를 수 있다(`Y`→`A`). (3) 2,000행 상한 안에서 기간을 정한다(월별 표는 최근 ~9년).
